...
class VariantSelects extends HTMLElement {
  ...
  variantChangeAlert(){
    ...
    alertHTML += `
      ...
      function splitAtLastDash(input) {
        const lastDashIndex = input.lastIndexOf("-");
        if (lastDashIndex === -1) {
            return [input]; // No dash found, return the entire string
        }
        const part1 = input.substring(0, lastDashIndex).trim();
        const part2 = input.substring(lastDashIndex + 1).trim();
        return [part1, part2];
      }
  }
  ...
  updateTitle(variantOb){
    ...
    */
    ...
    if(altTitlesArr){
      ...
      let titleToUse = splitFoundTitle == 'default' ? this.currentVariant.name.replace(` - ${this.currentVariant.title}`, "") : splitFoundTitle
  ...
  updateMedia() {
    ...
    document.getElementsByClassName('atc_mobile_image_container')[0].innerHTML = `<img src="${newMediaModal.src}" width="75" height="75"/>`
    document.getElementsByClassName('atc_desktop_image_container')[0].innerHTML = `<img src="${newMediaModal.src}" width="120" height="120"/>`
  ...
  renderProductInfo() {
    ...
    fetch(`${this.dataset.url}?variant=${requestedVariantId}&section_id=${this.dataset.originalSection ? this.dataset.originalSection : this.dataset.section}`)
      ...
      .then((responseText) => {
        ...
        const captureCtaSlideupPrice = html.getElementsByClassName('atc_price')[0];
